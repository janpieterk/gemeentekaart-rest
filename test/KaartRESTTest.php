...
class KaartRESTTest extends TestCase
{
    ...
    {
        $this->base_url = 'http://' . KAART_SERVER_HOSTNAME . KAART_SERVER_PATH;
        $this->ch = curl_init();
    ...
    public function testGetBitmapMunicipalities()
    {
        ...
        $url = $this->base_url . '?type=gemeentes&format=png';
        curl_setopt($this->ch, CURLOPT_URL, $url);
        curl_setopt($this->ch, CURLOPT_HTTPGET, 1);
        $kaart = curl_exec($this->ch);
    ...
    public function testCreateHighlightedMunicipalitiesMap()
    {
        $filename = substr(__FUNCTION__, 4) . '.png';
        $reference_image = KAART_REFERENCE_IMAGES_DIR . '/' . $filename;
        $url = $this->base_url . '?type=gemeentes&format=png&width=500';
    ...
    public function testCreateMunicipalitiesImagemapWithLinksNonInteractive()
    {
        ...
        $url = $this->base_url . '?type=gemeentes&format=png&imagemap=1';
    ...
    {
        $filename = substr(__FUNCTION__, 4) . '.html';
        $url = $this->base_url . '?type=gemeentes&format=png&imagemap=1&interactive=1';
        $this->setupJSONRequest($url, __DIR__ . '/data/complexmunicipalitieslist.json');
        $kaart = curl_exec($this->ch);
    ...
    {
        $filename = substr(__FUNCTION__, 4) . '.html';
        $url = $this->base_url . '?type=gemeentes&format=png&imagemap=1&linkhighlightedonly=1';
    ...
    public function testCreateMunicipalitiesSVGWithLinkNonInteractiveHighlightedOnly()
    {
        ...
        $url = $this->base_url . '?type=gemeentes&format=svg&&linkhighlightedonly=1';
        $this->setupJSONRequest($url, __DIR__ . '/data/complexmunicipalitieslist_link.json');
        $kaart = curl_exec($this->ch);
    ...
    {
        $filename = substr(__FUNCTION__, 4) . '.svg';
        $url = $this->base_url . '?type=municipalities&interactive=1&format=svg';
        curl_setopt($this->ch, CURLOPT_URL, $url);
        curl_setopt($this->ch, CURLOPT_HTTPGET, 1);
        $kaart = curl_exec($this->ch);
    ...
    public function testGetPossibleMunicipalities()
    {
        ...
        $url = $this->base_url . '?possiblemunicipalities=1';
        curl_setopt($this->ch, CURLOPT_URL, $url);
        curl_setopt($this->ch, CURLOPT_HTTPGET, 1);
        $kaart = curl_exec($this->ch);
    ...
    public function testBase64Map()
    {
        $filename = substr(__FUNCTION__, 4) . '.png';
        $reference_image = KAART_REFERENCE_IMAGES_DIR . '/' . $filename;
        $url = $this->base_url . '?type=dialectareas&format=png&width=300&base64=1';
        curl_setopt($this->ch, CURLOPT_URL, $url);
        curl_setopt($this->ch, CURLOPT_HTTPGET, 1);
        $kaart = curl_exec($this->ch);
    ...
    {
        $filename = substr(__FUNCTION__, 4) . '.json';
        $url = $this->base_url . '?possibleareas=1&type=corop';
        curl_setopt($this->ch, CURLOPT_URL, $url);
        curl_setopt($this->ch, CURLOPT_HTTPGET, 1);
        $kaart = curl_exec($this->ch);
        ...
        $url = $this->base_url . '?possibleareas=1&type=provincies';
        curl_setopt($this->ch, CURLOPT_URL, $url);
        curl_setopt($this->ch, CURLOPT_HTTPGET, 1);
        $kaart = curl_exec($this->ch);
        ...
        $url = $this->base_url . '?possibleareas=1';
        curl_setopt($this->ch, CURLOPT_URL, $url);
    ...
    public function testGetPossibleAreasWithYear()
    {
        $filename = substr(__FUNCTION__, 4) . '.json';
        $reference_image = KAART_REFERENCE_IMAGES_DIR . '/' . $filename;
        $url = $this->base_url . '?possibleareas=1&type=gemeentes&year=1933';
        curl_setopt($this->ch, CURLOPT_URL, $url);
        curl_setopt($this->ch, CURLOPT_HTTPGET, 1);
    ...
    public function testsetAdditionalAreasCorop()
    {
        $filename = substr(__FUNCTION__, 4) . '.png';
        $reference_image = KAART_REFERENCE_IMAGES_DIR . '/' . $filename;
        $url = $this->base_url . '?type=gemeentes&format=png&additionaldata=corop&width=800';
        curl_setopt($this->ch, CURLOPT_URL, $url);
        curl_setopt($this->ch, CURLOPT_HTTPGET, 1);
        $kaart = curl_exec($this->ch);
        $this->saveFile($filename, $kaart);
    ...
    public function testsetAdditionalAreasProvinces()
    {
        $filename = substr(__FUNCTION__, 4) . '.png';
        $reference_image = KAART_REFERENCE_IMAGES_DIR . '/' . $filename;
        $url = $this->base_url . '?type=gemeentes&format=png&additionaldata=provinces&width=800';
        curl_setopt($this->ch, CURLOPT_URL, $url);
        curl_setopt($this->ch, CURLOPT_HTTPGET, 1);
        $kaart = curl_exec($this->ch);
        $this->saveFile($filename, $kaart);
    ...
    public function testsetAdditionalAreasDialect()
    {
        $filename = substr(__FUNCTION__, 4) . '.png';
        $reference_image = KAART_REFERENCE_IMAGES_DIR . '/' . $filename;
        $url = $this->base_url . '?type=gemeentes&format=png&additionaldata=dialectareas&width=800';
        curl_setopt($this->ch, CURLOPT_URL, $url);
        curl_setopt($this->ch, CURLOPT_HTTPGET, 1);
        $kaart = curl_exec($this->ch);
        $this->saveFile($filename, $kaart);
    ...
    public function testHighlightedCorop()
    {
        $filename = substr(__FUNCTION__, 4) . '.png';
        $reference_image = KAART_REFERENCE_IMAGES_DIR . '/' . $filename;
        $url = $this->base_url . '?type=corop&format=png&width=500';
    ...
    public function testCreateHighlightedMunicipalitiesNLFlandersMap()
    {
        $filename = substr(__FUNCTION__, 4) . '.png';
        $reference_image = KAART_REFERENCE_IMAGES_DIR . '/' . $filename;
        $url = $this->base_url . '?type=municipalities_nl_flanders&format=png';
    ...
    public function testVragenlijstenKaart()
    {
        $filename = substr(__FUNCTION__, 4) . '.png';
        $reference_image = KAART_REFERENCE_IMAGES_DIR . '/' . $filename;
        $url = $this->base_url . '?type=gemeentes&format=png&width=600&additionaldata=corop';
    ...
    public function testOutlinedProvincie()
    {
        $filename = substr(__FUNCTION__, 4) . '.png';
        $reference_image = KAART_REFERENCE_IMAGES_DIR . '/' . $filename;
        $url = $this->base_url . '?type=gemeentes&format=png&width=600&additionaldata=provincies';
    ...
    public function testCSVFileUpload()
    {
        $filename = substr(__FUNCTION__, 4) . '.png';
        $reference_image = KAART_REFERENCE_IMAGES_DIR . '/' . $filename;
        $url = $this->base_url . '?type=dialectareas&format=png&width=400';
    ...
    {
        $filename = substr(__FUNCTION__, 4) . '.svg';
        $url = $this->base_url . '?type=municipalities&format=svg&interactive=1&pathsfile=alternate_paths.json';
        curl_setopt($this->ch, CURLOPT_URL, $url);
        curl_setopt($this->ch, CURLOPT_HTTPGET, 1);
        $kaart = curl_exec($this->ch);
    ...
    {
        $url = $this->base_url . '?possibletypes=1';
        curl_setopt($this->ch, CURLOPT_URL, $url);
        curl_setopt($this->ch, CURLOPT_HTTPGET, 1);
        $actual = curl_exec($this->ch);
    ...
    {
        $url = $this->base_url . '?possibleformats=1';
        curl_setopt($this->ch, CURLOPT_URL, $url);
        curl_setopt($this->ch, CURLOPT_HTTPGET, 1);
        $actual = curl_exec($this->ch);
    ...
    {
        $url = $this->base_url . '?type=provincies&possibleyears=1';
        curl_setopt($this->ch, CURLOPT_URL, $url);
        curl_setopt($this->ch, CURLOPT_HTTPGET, 1);
        $actual = curl_exec($this->ch);
     ...
     * @throws ImagickException
     */
    public function testGetBitmapMunicipalities1860()
    {
        $filename = substr(__FUNCTION__, 4) . '.png';
        $reference_image = KAART_REFERENCE_IMAGES_DIR . '/' . $filename;
        $url = $this->base_url . '?type=gemeentes&format=png&year=1860';
        curl_setopt($this->ch, CURLOPT_URL, $url);
        curl_setopt($this->ch, CURLOPT_HTTPGET, 1);
        $kaart = curl_exec($this->ch);
        $this->saveFile($filename, $kaart);
        ...
        public function testCreateRandstadSVG1821()
    {
        $filename = substr(__FUNCTION__, 4) . '.svg';
        $url = $this->base_url;
        $this->setupJSONRequest($url, __DIR__ . '/data/randstad1821.json');
        $kaart = curl_exec($this->ch);
        $expected = 'be76fe441a56b1be6026413616cd9132';
        $actual = md5($this->saveFile($filename, $kaart));
        $this->assertEquals($expected, $actual, "check file " . KAART_TESTDIRECTORY . "/$filename");
    }
